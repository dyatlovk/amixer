 ...
 */
export default function SvgKnob(elem, conf = {}) {
  ...
  const HALF_WIDTH = 20 // viewBox/2
  const HALF_HEIGHT = 20 // viewBox/2
  ...
  let defaults = {
    ...
    angle_min: 40.0, // [deg] Angle in knob coordinates (0 at 6 0'clock)
    angle_max: 320.0, // [deg] Angle in knob coordinates (0 at 6 0'clock)
    ...
    value_text: true,
    value_position: HALF_HEIGHT + 20, // empirical value: HALF_HEIGHT + config.font_size / 3
  ...
  svg_element.setAttributeNS(
    null,
    'viewBox',
    `0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_WIDTH}`
  )
  ...
  function init() {
    ...
    if (config.center_zero) {
      if (!config.center_value) {
        config.center_value = getRoundedValue(
          (config.value_max - config.value_min) / 2 + config.value_min
        )
      }
    }
    ...
    if (config.linecap === 'butt') {
      left_track_end_angle = polarToKnobAngle(
        (Math.acos(-config.center_gap / 100.0) * 180.0) / Math.PI
      )
      right_track_start_angle = polarToKnobAngle(
        (Math.acos(config.center_gap / 100.0) * 180.0) / Math.PI
      )
    } else {
      left_track_end_angle = polarToKnobAngle(
        (Math.acos(-(config.track_width * 1.3 + config.center_gap) / 100.0) *
          180.0) /
          Math.PI
      )
      right_track_start_angle = polarToKnobAngle(
  ...
  function getRoundedValue(v) {
    return config.value_resolution === null
      ? v
      : Math.round(v / config.value_resolution) * config.value_resolution
  }
  ...
  function getValue(a) {
    ...
    let v =
      ((p - config.angle_min) / (config.angle_max - config.angle_min)) *
  ...
  function setValue(v) {
    ...
    setAngle(
      ((v + getCursorCorrection() - config.value_min) /
        (config.value_max - config.value_min)) *
    ...
    if (trace)
      console.log(
        `setValue(${v}) angle=` +
          ((v - config.value_min) / (config.value_max - config.value_min)) *
            (config.angle_max - config.angle_min) +
          config.angle_min
      )
    return true
  ...
  function incAngle(increment) {
    setAngle(
      Math.min(Math.max(angle + increment, config.angle_min), config.angle_max),
      true
    )
  }
    ...
    if (trace)
      console.log(
        `polarToKnobAngle ${angle} -> ${
          (config.zero_at - angle + 360.0) % 360.0
        }`
      )
  ...
  function mouseUpdate(e) {
    ...
    if (trace)
      console.log(
        `mouseUpdate: position in svg = ${dxPixels}, ${dyPixels} pixels; ${dx.toFixed(
          3
        )}, ${dy.toFixed(3)} rel.; angle ${angle_rad.toFixed(3)} rad`
      )
  ...
  function mouseWheelHandler(e) {
    ...
    incAngle(
      (dy / minDeltaY) * mouse_wheel_direction * config.mouse_wheel_acceleration
    )
  ...
  function handleTouch(e) {
    ...
    if (trace)
      console.log(
        `handleTouch: position in svg = ${dxPixels}, ${dyPixels} pixels; ${dx.toFixed(
          3
        )}, ${dy.toFixed(3)} rel.; angle ${angle_rad.toFixed(3)} rad`
      )
    ...
    if (config.center_zero) {
      if (Array.isArray(config.center_value)) {
        ...
        if (config.center_value.includes(getValue())) {
          if (trace)
            console.log('getTrackPath: center position, track not drawn')
      ...
      if (angle < 180) {
        p = getArc(
          Math.min(angle, left_track_end_angle),
          left_track_end_angle,
          config.track_radius
        )
      } else if (angle > 180) {
        p = getArc(
          right_track_start_angle,
          Math.max(angle, right_track_start_angle),
          config.track_radius
        )
      }
    } else {
  ...
  function draw_markers() {
    ...
    for (let a = config.angle_min; a <= config.angle_max; a += step) {
      ...
      let to = getViewboxCoord(
        knobToPolarAngle(a),
        config.markers_radius + config.markers_length
      )
...
*/
  ...
  function draw_track_background() {
    ...
    if (config.center_zero) {
      ...
      svg_track_bg_left.setAttributeNS(
        null,
        'd',
        getArc(config.angle_min, left_track_end_angle, config.track_bg_radius)
      )
      ...
      svg_track_bg_right.setAttributeNS(
        null,
        'd',
        getArc(
          right_track_start_angle,
      ...
      svg_track_bg_right.setAttribute(
        'stroke-width',
        `${config.track_bg_width}`
      )
      ...
      svg_track_bg.setAttributeNS(
        null,
        'd',
        getArc(config.angle_min, config.angle_max, config.track_bg_radius)
      )
  ...
  function update_value() {
    ...
    if (typeof t === 'number' || t.indexOf('\n') <= 0) {
      ...
      if (svg_value_text) {
        svg_value_text.setAttributeNS(
          null,
          'y',
          `${config.value_position - 12}`
        )
        svg_value_text.setAttribute(
          'font-size',
          `${getFontSize(lines[0], true)}`
        )
        svg_value_text.textContent = lines[0]
      }

      if (svg_value_text2) {
        svg_value_text2.setAttributeNS(
          null,
          'y',
          `${config.value_position + 4}`
        )
        svg_value_text2.setAttribute(
          'font-size',
          `${getFontSize(lines[1], true)}`
        )
